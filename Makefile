...
run: compile
	bash -c '/usr/bin/time -f "\nelapsed: %es\nrss: %Mk" ./fastpermt run --method sum --graph-file aux/graph -c 20 --ignore-label aux/ft_ignore-lh.label $$(ls data/ | cut -f1,2 -d_ | uniq | xargs -I{} echo data/{}_45_control-lh.stc data/{}_45_kanizsa-lh.stc) -o aux/result.stc'
